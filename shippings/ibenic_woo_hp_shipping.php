		...
		if ( ! class_exists( 'WC_HP_Shipping' ) ) {
			class WC_HP_Shipping extends WC_Shipping_Method {
				...
				public function __construct( $instance_id = 0 ) {
					...
					$this->init();
					$countries = array_keys( ibenic_get_hp_shipping_countries() );
					$countries[] = 'HR';
					$this->countries = $countries;
					$this->availiability = 'including';
...
function hide_all_shipping_when_free_is_available( $rates, $package ) {
	...
	$excluded_states = array( 'HR');
 
	if( isset( $rates['wc_hp_shipping'] ) AND in_array( WC()->customer->shipping_country, $excluded_states ) ) {
 
		// remove free shipping option
		unset( $rates['wc_hp_shipping'] );
